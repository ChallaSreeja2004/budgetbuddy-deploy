...
const Dashboard = () => {
  ...
  const fetchTransactions = async (page = 1) => {
    ...
    try {
      ...
      const response = await axios.get(`https://budgetbuddy-deploy.vercel.app/api/transactions?${params}`);
  ...
  const handleAddTransaction = async (data) => {
    await axios.post('https://budgetbuddy-deploy.vercel.app/api/transactions', data);
    setFilters({ type: 'all', category: 'all' }); // Reset filters to force a full refresh
    navigate('/');
  };
  const handleDeleteTransaction = async (id) => {
    await axios.delete(`https://budgetbuddy-deploy.vercel.app/api/transactions/${id}`);
    fetchTransactions(pagination.currentPage); // Refetch the current page
  };
  const handleUpdateTransaction = async (id, data) => {
    await axios.put(`https://budgetbuddy-deploy.vercel.app/api/transactions/${id}`, data);
    navigate('/');
